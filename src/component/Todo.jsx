...
const Todo = () => {
    ...
    const handleAdd=()=>{
        ...
        if(edit !==null){
            ...
            updateTodo[edit]={...updateTodo[edit],data:data}
            setTodo(updateTodo);
            setEdit(null); 
        }
        else{
  ...
  return (
    ...
    <div className='mx-auto text-center my-7'>
        ...
        <button className='bg-black text-white p-2 rounded' onClick={handleAdd}>{edit !== null? 'update':'add'}</button>
        <ul>
            {todo.map((todos,index)=>(
                <li key={index} className='mb-2'>{todos.data}
                <button className='bg-green-500 p-2 text-white rounded mx-2' onClick={()=>handleEdit(index)}>edit</button>
